...
router.get("/random", randomRateLimiter, async (req, res) => {
  ...
  if (Math.random() >= 0.5) {
    ...
    if (sauce == null) {
      // If no ImageSauce found, try to get a random QuoteSauce
      sauce = await MongoDBService.getRandomQuoteSauce();

      if (sauce == null) {
        // If no QuoteSauce found either
  ...
  if (sauce == null) {
    // If no QuoteSauce found, try to get a random ImageSauce
...
router.post("/image", postRateLimiter, async (req, res) => {
  if (req.files === undefined || req.files.image === undefined) {
    // Image type request, no images passed in
    return res.status(400).json({
